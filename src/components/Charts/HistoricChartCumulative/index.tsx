  ...
  useEffect(() => {
    for (const key in props.month) {
      ...
      cagos.forEach((cago: any) => {
        ...
        if (chartcago.length) {
          if (chartcago[0].data.length)
            chartcago[0].data.push(
              chartcago[0].data[chartcago[0].data.length - 1] + cago.cacas
            );
          else chartcago[0].data.push(cago.cacas);
        } else {
